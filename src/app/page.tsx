  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-[600px] bg-gradient-to-b from-violet-50 to-transparent -z-10" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            {t.landing.heroTitle.split(' ').map((word, i) => (
              <span key={i} className={word === 'AI' ? 'text-violet-700' : ''}>{word} </span>
            ))}
          </motion.h1>
          ...
          >
            <Link
              href="/generate"
              className="bg-violet-700 text-white px-8 py-4 rounded-full text-lg font-bold hover:bg-violet-800 transition-all shadow-xl shadow-violet-200 flex items-center gap-2"
            >
          ...
          <div className="grid md:grid-cols-3 gap-12">
            {steps.map((step, index) => (
              <div key={index} className="relative group text-center">
                <div className="w-16 h-16 bg-violet-100 text-violet-700 rounded-2xl flex items-center justify-center mx-auto mb-6 transform group-hover:rotate-6 transition-transform">
                  {step.icon}
                </div>
      ...
      <footer className="py-12 border-t border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="text-2xl font-bold font-lexend text-violet-700">LakuAI</div>
          <div className="text-gray-500 text-sm">
            © {new Date().getFullYear()} LakuAI. Made for Malaysian Sellers.
          </div>
        </div>
      </footer>
    </main>
